...
with col2:
    if data:
        # Check if the column is categorical (strings)
        if not all(isinstance(x, (int, float)) for x in data):
            data = [str(x) for x in data]  # Ensure all are strings

            # Calculate frequency for string values
            frequency_dict = {}
            for item in data:
            ...
            st.subheader("String Frequency Table")
            st.table(freq_df)

            # Pareto chart for strings
            ...
            # Histogram for strings (as bar chart)
            ...
            st.stop()  # Skip numeric interval logic if it's a string column
