...
with st.sidebar:
                    ...
                    if first_non_empty_col_index is None:
                        ...
                        for idx, col in enumerate(df.columns):
                            if idx < first_non_empty_col_index:
                                continue
                            col_data = df[col]
                            if selected_type == "Categorical" and is_categorical_column(col_data):
                                column_mapping[get_excel_column_name(idx)] = col
                            elif selected_type == "Numerical" and is_numerical_column(col_data):
                                column_mapping[get_excel_column_name(idx)] = col
    ...
    # Show number of intervals input only for Numerical type
    if selected_type == "Numerical":
...
with col2:
    if data:
        if selected_type == "Categorical":
            ...
            if num_intervals and num_intervals.isdigit():
                try:
